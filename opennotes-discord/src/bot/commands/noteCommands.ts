...
export class NoteCommands {
  private communityNoteService: OpenNoteService;
  ...
  async handleNoteRatingButton(interaction: ButtonInteraction): Promise<void> {
    ...
    try {
      ...
      if (note.authorId === user.id) {
        await interaction.editReply({
          content: '❌ You cannot rate your own open note.',
        });
        return;
      }
   ...
   */
  private createNoteNavigationButtons(notes: OpenNoteWithRelations[], currentIndex: number): ActionRowBuilder<ButtonBuilder> {

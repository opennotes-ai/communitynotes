...
export class RequestorScoringAdapter {
  ...
  async isUserEligibleForRequest(
    userId: string,
    serverId: string
  ): Promise<{ eligible: boolean; reason?: string; remainingRequests?: number }> {
    try {
   ...
   */
  async getRequestorMetrics(userId: string) {
   ...
   */
  async calculateRequestorHelpfulness(userId: string) {
    try {
      const metrics = await openNotesClient.getRequestorMetrics(userId);
      return {
        score: metrics.score,
...
export class ScoringAdapter {
   ...
   */
  async calculateNoteScore(noteId: string) {
    try {
   ...
   */
  async performBulkScoring(noteIds?: string[], userIds?: string[]) {
    try {

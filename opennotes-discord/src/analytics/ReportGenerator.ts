...
import type { AnalyticsReport, ConversionMetrics, EngagementMetrics, TimeSeriesData } from './types.js';
...
export class ReportGenerator {
  ...
  async generatePDF(report: AnalyticsReport): Promise<string> {
    try {
  ...
  async generateCSV(report: AnalyticsReport): Promise<string> {
    try {
  ...
  async generateJSON(report: AnalyticsReport): Promise<string> {
    try {
  ...
  async generateAllFormats(report: AnalyticsReport): Promise<AnalyticsReport> {
    try {
      await Promise.all([
...
<body>
  ...
  private generateCSVContent(report: AnalyticsReport): string {
    const { data } = report;
    let csv = '';
  ...
  private generateServerBreakdownTable(metrics: ConversionMetrics): string {
    if (metrics.breakdown.byServer.length === 0) return '';
  ...
  private generateTopContributorsTable(metrics: EngagementMetrics): string {
    if (metrics.topContributors.length === 0) return '';
  ...
  private generateStatusBreakdownTable(metrics: any): string {

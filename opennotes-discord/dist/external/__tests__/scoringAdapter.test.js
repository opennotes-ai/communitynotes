...
import { openNotesClient, setMockRequestorMetrics, setMockNoteScore, setMockUserScore, resetMocks } from '../__mocks__/opennotesClient.js';
...
describe('RequestorScoringAdapter', () => {
    ...
    describe('trackEligibleRequest', () => {
        it('should submit request to external service', async () => {
            await adapter.trackEligibleRequest('user-1', 'msg-123', 'Misleading information', ['https://example.com']);
        ...
        it('should handle submission failures gracefully', async () => {
            openNotesClient.shouldFailRequests = true;
            // Should not throw
            await expect(adapter.trackEligibleRequest('user-1', 'msg-123')).resolves.toBeUndefined();
        });
    });
...
describe('ScoringAdapter', () => {
    ...
    describe('performBulkScoring', () => {
        it('should request bulk scoring from external service', async () => {
            ...
            const result = await adapter.performBulkScoring(['note-1', 'note-2'], ['user-1']);
...
//# sourceMappingURL=scoringAdapter.test.js.map
...
export class RedisVsNatsBenchmark {
    ...
    async benchmarkRedisPublish(messageCount) {
    ...
    async benchmarkNatsPublish(messageCount) {
        const operation = `Publish ${messageCount} messages`;
        const latencies = [];
        let errors = 0;
        const testData = this.generateTestMessage();
    ...
    async benchmarkRedisTimeSeries(messageCount) {
    ...
    async benchmarkNatsTimeSeries(messageCount) {
        const operation = `Time Series ${messageCount} entries`;
        const latencies = [];
        let errors = 0;
        const startTime = Date.now();
    ...
    generateTestMessage() {
        ...
        if (currentSize < this.MESSAGE_SIZE_BYTES) {
            message.data.metadata['padding'] = 'x'.repeat(this.MESSAGE_SIZE_BYTES - currentSize - 20);
        }
        return message;
    }
    ...
    async getBenchmarkReport(results) {
...
//# sourceMappingURL=RedisVsNatsBenchmark.js.map
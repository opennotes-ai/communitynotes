...
import { MessagePriority } from '../streaming/StreamingService.js';
import { prisma } from '../database/client.js';
export class NatsNotificationService {
    sender;
    isProcessing = false;
    NOTIFICATION_SUBJECT = 'notifications.queue';
    BATCH_SUBJECT = 'notifications.batch';
    ...
    async start() {
        ...
        try {
            ...
            await jetStreamService.createConsumer(this.NOTIFICATION_SUBJECT, 'notification-processor', this.processNotificationMessage.bind(this), {
                durable: 'notification-processor',
                maxDeliver: 3,
                ackWait: 30000
            });
            // Subscribe to batch notifications
            await jetStreamService.createConsumer(this.BATCH_SUBJECT, 'batch-processor', this.processBatchMessage.bind(this), {
    ...
    async queueNotification(userId, type, data, priority = NotificationPriority.MEDIUM, scheduledFor) {
        try {
    ...
    async processNotificationMessage(message) {
        ...
        try {
            const notificationMsg = message.data;
            ...
            let success = false;
            const methods = user.notificationMethods;
    ...
    async processBatchMessage(message) {
        try {
            const batchMsg = message.data;
    ...
    isNotificationTypeEnabled(user, type) {
        switch (type) {
    ...
    async getUserPreferences(userId) {
        try {
            ...
            return {
                ...
                methods: user.notificationMethods,
...
//# sourceMappingURL=NatsNotificationService.js.map
...
export class AnalyticsService {
    ...
    async getEngagementMetrics(filter) {
        try {
            ...
            const [totalUsers, activeUsers, contributorCount, raterCount, userGrowth, dailyActiveUsers, contributorActivity, ratingActivity, topContributors] = await Promise.all([
    ...
    async getNoteEffectiveness(filter) {
        try {
            const { start, end, serverId } = filter;
            const whereCondition = {
                submittedAt: { gte: start, lte: end },
            ...
            const [notes, visibleNotes, hiddenNotes, statusBreakdown, helpfulnessBreakdown, helpfulnessTrend, visibilityTrend] = await Promise.all([
    ...
    async getSystemPerformance(filter) {
        try {
    ...
    async generateReport(filter, title) {
        try {
            const reportId = `report_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
            const [conversionMetrics, engagementMetrics, noteEffectiveness, systemPerformance, serverInfo] = await Promise.all([
    ...
    async getRequestsByTimeframe(start, end, granularity, serverId) {
        const whereCondition = {
            timestamp: { gte: start, lte: end },
            ...(serverId && { message: { serverId } })
        };
    ...
    async getNotesByTimeframe(start, end, granularity, serverId) {
        const whereCondition = {
            submittedAt: { gte: start, lte: end },
            ...(serverId && { message: { serverId } })
        };
    ...
    async getHourlyConversionData(start, end, serverId) {
    ...
    async getServerBreakdown(start, end, serverId) {
        if (serverId) {
    ...
    async getActiveUsers(start, end, serverId) {
        const whereCondition = {
    ...
    async getContributorCount(start, end, serverId) {
        const whereCondition = {
            submittedAt: { gte: start, lte: end },
            ...(serverId && { message: { serverId } })
        };
    ...
    async getRaterCount(start, end, serverId) {
        const whereCondition = {
            timestamp: { gte: start, lte: end },
            ...(serverId && {
                note: {
    ...
    async getUserGrowthTrend(start, end, serverId) {
        try {
    ...
    async getDailyActiveUsersTrend(start, end, serverId) {
        try {
    ...
    async getContributorActivityTrend(start, end, serverId) {
        try {
    ...
    async getRatingActivityTrend(start, end, serverId) {
        try {
    ...
    async getTopContributors(start, end, serverId, limit = 10) {
        const whereCondition = {
            submittedAt: { gte: start, lte: end },
            ...(serverId && { message: { serverId } })
        };
    ...
    async getNoteStatusBreakdown(start, end, serverId) {
        const whereCondition = {
            submittedAt: { gte: start, lte: end },
            ...(serverId && { message: { serverId } })
        };
    ...
    async getHelpfulnessBreakdown(start, end, serverId) {
        const whereCondition = {
            submittedAt: { gte: start, lte: end },
            totalRatings: { gt: 0 },
            ...(serverId && { message: { serverId } })
    ...
    async getHelpfulnessTrend(start, end, serverId) {
        try {
    ...
    async getVisibilityTrend(start, end, serverId) {
        try {
    ...
    async getResponseTimeTrend(start, end) {
        try {
    ...
    async getThroughputTrend(start, end) {
        try {
    ...
    async getErrorRateTrend(start, end) {
        try {
    ...
    aggregateByTimeframe(timestamps, granularity) {
        const data = new Map();
    ...
    mergeTimeSeriesData(requestsData, notesData) {

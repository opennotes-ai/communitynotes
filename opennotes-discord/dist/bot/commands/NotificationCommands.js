...
export class NotificationCommands {
    ...
    async handleNotificationCommand(interaction) {
    ...
    async handleSettingsCommand(interaction, userId) {
    ...
    async showNotificationTypeToggle(interaction, userId) {
        const preferences = await this.notificationService.getUserPreferences(userId);
        if (!preferences)
            return;
        ...
        const selectCollector = interaction.channel?.createMessageComponentCollector({
            ...
            filter: (i) => i.user.id === interaction.user.id && i.customId === 'toggle_notification_types',
            time: 2 * 60 * 1000
        });
    ...
    async showMethodSelection(interaction, userId) {
        const preferences = await this.notificationService.getUserPreferences(userId);
        if (!preferences)
            return;
        const selectMenu = new StringSelectMenuBuilder()
        ...
        const selectCollector = interaction.channel?.createMessageComponentCollector({
            componentType: ComponentType.StringSelect,
            filter: (i) => i.user.id === interaction.user.id && i.customId === 'set_notification_methods',
            time: 2 * 60 * 1000
        });
        selectCollector?.on('collect', async (selectInteraction) => {
            const selectedMethods = selectInteraction.values;
    ...
    async showBatchingConfig(interaction, userId) {
        const preferences = await this.notificationService.getUserPreferences(userId);
        if (!preferences)
            return;
        const selectMenu = new StringSelectMenuBuilder()
        ...
        const selectCollector = interaction.channel?.createMessageComponentCollector({
            componentType: ComponentType.StringSelect,
            filter: (i) => i.user.id === interaction.user.id && i.customId === 'set_batching_config',
            time: 2 * 60 * 1000
        });
        selectCollector?.on('collect', async (selectInteraction) => {
    ...
    async handleMuteCommand(interaction, userId) {
    ...
    async handleUnmuteCommand(interaction, userId) {
    ...
    async handleTestCommand(interaction, userId) {
        const testData = {
            messageUrl: 'https://discord.com/channels/test/test/test',
            noteId: 'test-note-123'
        };
        const notificationId = await this.notificationService.queueNotification(userId, 'contributor_milestone_reached', {

...
export class AdminService {
    ...
    // Channel management
    async enableChannel(serverId, channelId, adminId) {
        try {
    ...
    async disableChannel(serverId, channelId, adminId) {
        try {
            const server = await prisma.server.findUnique({
                where: { id: serverId },
                select: { enabledChannels: true, disabledChannels: true },
    ...
    async addContributor(serverId, userId, roleId, adminId) {
        try {
            const server = await prisma.server.findUnique({
                where: { id: serverId },
    ...
    async removeContributor(serverId, userId, adminId) {
        try {
    ...
    // Emergency controls
    async pauseSystem(serverId, adminId, reason) {
        try {
            await prisma.server.update({
                where: { id: serverId },
                data: {
    ...
    async resumeSystem(serverId, adminId) {
        try {
            await prisma.server.update({
                where: { id: serverId },
                data: {
    ...
    async getModerationQueue(serverId, status) {
        try {
    ...
    async reviewModerationItem(id, reviewerId, action, actionTaken) {
        try {
    ...
    // Bulk actions
    async bulkDeleteNoteRequests(serverId, messageIds, adminId) {
        try {
    ...
    async bulkDeleteOpenNotes(serverId, noteIds, adminId) {
        try {
            const result = await prisma.openNote.deleteMany({
                where: {
                    id: { in: noteIds },
                    message: { serverId },
                },
            ...
            logger.error('Error bulk deleting open notes:', error);
            throw error;
        }
    }
    // Statistics and monitoring
    async getAdminStats(serverId) {
        try {
            const [totalRequests, totalNotes, pendingModeration, recentActions, topContributors,] = await Promise.all([
    ...
    async getTopContributors(serverId) {
        try {
    ...
    // Get audit logs
    async getAuditLogs(serverId, limit = 50) {
        try {

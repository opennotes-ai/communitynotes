...
export function createDashboardRoutes(verificationService) {
    // Authentication middleware for dashboard
    const authenticateContributor = async (req, res, next) => {
        try {
    ...
    router.get('/feed', authenticateContributor, async (req, res) => {
        try {
            const { serverId, channelId, timeframe = '24', minRequests = '1', status = 'pending', limit = '50', offset = '0' } = req.query;
            const timeframeHours = parseInt(timeframe);
            const minRequestCount = parseInt(minRequests);
            const requestLimit = Math.min(parseInt(limit), 100);
            const requestOffset = parseInt(offset);
            ...
            for (const server of userServers) {
                const messages = await messageService.getMessagesNeedingNotes(server.id, minRequestCount, requestLimit);
                for (const message of messages) {
                    ...
                    const recentRequestsInTimeframe = recentRequests.filter(req => req.timestamp >= cutoffTime);
                        ...
                        messagesWithRequests.push({
                            ...
                            requests: recentRequestsInTimeframe.map(req => ({
                                ...
                                requestor: req.requestor ? {
                                    ...
                                    trustLevel: req.requestor.trustLevel,
                                    helpfulnessScore: req.requestor.helpfulnessScore
                                } : null
                            }))
                        });
                    }
    ...
    router.get('/message/:messageId', authenticateContributor, async (req, res) => {
        try {
    ...
    router.get('/servers', authenticateContributor, async (req, res) => {
        try {
    ...
    router.get('/stats', authenticateContributor, async (req, res) => {
        try {
            ...
            const serverStats = servers.map(server => {
                const serverRequests = recentRequests.filter(req => req.message?.serverId === server.id);
                return {
                    serverId: server.id,
            ...
            for (let i = hours - 1; i >= 0; i--) {
                ...
                const hourRequests = recentRequests.filter(req => req.timestamp >= hourStart && req.timestamp < hourEnd);
                hourlyTrends.push({
    ...
    router.post('/notes', authenticateContributor, async (req, res) => {
        try {
    ...
    // Get user's notes
    router.get('/notes/my', authenticateContributor, async (req, res) => {
        try {
            const { limit = '20', offset = '0' } = req.query;
            const noteLimit = Math.min(parseInt(limit), 100);
            const noteOffset = parseInt(offset);
    ...
    router.get('/notes/:noteId', authenticateContributor, async (req, res) => {
        try {
            ...
            res.json({
                note: {
                    ...
                    author: note.author ? {
                        ...
                        trustLevel: note.author.trustLevel,
                        helpfulnessScore: note.author.helpfulnessScore
                    } : null,
                    message: note.message ? {
                        id: note.message.discordId,
                        content: note.message.content,
                        channelId: note.message.channelId,
                        timestamp: note.message.timestamp,
                        server: note.message.server ? {
                            name: note.message.server.name,
                            discordId: note.message.server.discordId
                        } : null
                    } : null,
                    ratings: note.ratings ? note.ratings.map((rating) => ({
                        id: rating.id,
    ...
    router.delete('/notes/:noteId', authenticateContributor, async (req, res) => {
        try {
            const { noteId } = req.params;
            const note = await communityNoteService.findById(noteId);
            if (!note) {
    ...
    // Moderation Queue
    router.get('/admin/moderation', authenticateAdmin, async (req, res) => {
        try {
            ...
            const queue = await adminService.getModerationQueue(server.id, status);
            res.json(queue);
        }
        catch (error) {
    ...
    router.patch('/admin/moderation/:itemId', authenticateAdmin, async (req, res) => {
        try {
    ...
    // Channel Management
    router.patch('/admin/channels', authenticateAdmin, async (req, res) => {
        try {
    ...
    // Emergency Controls
    router.post('/admin/emergency', authenticateAdmin, async (req, res) => {
        try {

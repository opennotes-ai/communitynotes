...
import { z } from 'zod';
import { StartVerificationRequestSchema, CompleteVerificationRequestSchema, } from '../../shared/types/verification.js';
import { logger } from '../../shared/utils/logger.js';
export function createVerificationRoutes(verificationService, verificationMiddleware) {
    const router = Router();
    ...
    router.post('/start', rateLimitMiddleware, async (req, res) => {
        try {
    ...
    router.post('/complete', rateLimitMiddleware, async (req, res) => {
        try {
    ...
    router.get('/status/:discordUserId', async (req, res) => {
        try {
    ...
    router.get('/permissions/:discordUserId', async (req, res) => {
        try {
            const { discordUserId } = req.params;
            const requestingUserId = req.headers['x-discord-user-id'];
    ...
    router.post('/admin/verify', verificationMiddleware.requireModerator(), async (req, res) => {
        try {

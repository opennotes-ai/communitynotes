...
export class HealthCheckService {
    ...
    determineOverallStatus(services) {
        ...
        const hasAnyFailure = Object.values(services).some(service => service.status === 'unhealthy' || service.status === 'critical' || service.status === 'warning');
        if (hasAnyFailure) {
            return 'degraded';
        }
        return 'healthy';
    }
    async getDetailedDatabaseStats() {
        try {

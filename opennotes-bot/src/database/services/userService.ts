...
export class UserService {
  ...
  async updateUser(
    discordId: string,
    data: Partial<Pick<User, 'username' | 'discriminator' | 'avatar' | 'lastActiveAt'>>
  ): Promise<User> {
    try {
  ...
  async updateHelpfulnessScore(userId: string, scoreChange: number): Promise<User> {
    try {
      return await prisma.user.update({
        where: { id: userId },
        data: {
  ...
  async incrementDailyRequestCount(userId: string): Promise<User> {
    try {
  ...
  async updateTrustLevel(userId: string, trustLevel: string): Promise<User> {
    try {
      return await prisma.user.update({
        where: { id: userId },
        data: { trustLevel },
  ...
  async incrementNoteCount(userId: string): Promise<User> {
    try {
      return await prisma.user.update({
        where: { id: userId },
        data: {

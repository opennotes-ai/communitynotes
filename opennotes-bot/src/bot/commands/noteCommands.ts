...
import { CommunityNoteService, NoteRatingService, MessageService, UserService } from '../../database/services/index.js';
...
export class NoteCommands {
  ...
  private noteRatingService: NoteRatingService;
  ...
  getViewNotesCommand(): SlashCommandBuilder {
    return new SlashCommandBuilder()
      .setName('view-notes')
      .setDescription('View community notes for a specific message')
   ...
   */
  async handleViewNotesCommand(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      ...
      if (!message) {
        await interaction.editReply({
          content: '❌ No message found with that ID. Make sure the message has community notes or requests.',
        });
        return;
      }
   ...
   */
  async handleNoteRatingButton(interaction: ButtonInteraction): Promise<void> {
   ...
   */
  private async handleViewSources(interaction: ButtonInteraction, noteId: string): Promise<void> {
    try {
      const note = await this.communityNoteService.findById(noteId);
      if (!note) {
        await interaction.reply({
   ...
   */
  private createNoteNavigationButtons(notes: CommunityNoteWithRelations[], currentIndex: number): ActionRowBuilder<ButtonBuilder> {

...
export class AdminCommands {
  ...
  public static getSlashCommand() {
    return new SlashCommandBuilder()
      .setName('configure')
      .setDescription('Configure Community Notes settings (Admin only)')
      .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
      .addSubcommand(
        new SlashCommandSubcommandBuilder()
          .setName('enable')
          .setDescription('Enable Community Notes in a channel')
          .addChannelOption(
      ...
      .addSubcommand(
        new SlashCommandSubcommandBuilder()
          .setName('disable')
          .setDescription('Disable Community Notes in a channel')
          .addChannelOption(
            new SlashCommandChannelOption()
              .setName('channel')
      ...
      .addSubcommand(
        new SlashCommandSubcommandBuilder()
          .setName('stats')
          .setDescription('View Community Notes statistics and activity')
      )
      .addSubcommand(
        new SlashCommandSubcommandBuilder()
          .setName('moderation')
  ...
  public async handleConfigureCommand(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      ...
      if (!server) {
        await interaction.reply({
          content: '❌ Community Notes is not set up for this server. Please contact support.',
          ephemeral: true,
        });
        return;
      }
  ...
  private async handleEnableChannel(
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(`Community Notes are now enabled in ${channel}.`)
        .addFields({
          name: '📝 What this means',
          value: '• Users can now request Community Notes on messages in this channel\n• Contributors can write and rate notes\n• All server settings apply to this channel',
          inline: false,
        })
        .setTimestamp();
  ...
  private async handleDisableChannel(
    interaction: ChatInputCommandInteraction,
    serverId: string,
    adminId: string
  ): Promise<void> {
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(`Community Notes are now disabled in ${channel}.`)
        .addFields({
          name: '⚠️ What this means',
          value: '• Users cannot request new Community Notes in this channel\n• Existing notes will remain visible\n• Contributors cannot write new notes for messages in this channel',
          inline: false,
        })
        .setTimestamp();
  ...
  private async handleContributor(
    interaction: ChatInputCommandInteraction,
    serverId: string,
    adminId: string
  ): Promise<void> {
    ...
    try {
      if (action === 'pause') {
        ...
        const embed = new EmbedBuilder()
          ...
          .setDescription('Community Notes system has been resumed.')
          .addFields({
            name: '✅ System Status',
            value: '• All Community Notes functionality is now active\n• Users can request notes on messages\n• Contributors can write and rate notes\n• Normal operations have resumed',
            inline: false,
          })
          .setTimestamp();
  ...
  private async handleStats(interaction: ChatInputCommandInteraction, serverId: string): Promise<void> {
    try {
      ...
      const embed = new EmbedBuilder()
        .setColor(0x0099ff)
        .setTitle('📊 Community Notes Statistics')
        .addFields(
          {
            name: '📈 Overview',
            value: `**${stats.totalRequests}** Total Requests\n**${stats.totalNotes}** Community Notes\n**${stats.pendingModeration}** Pending Moderation`,
            inline: true,
          },
          {
            name: '👥 Top Contributors',
            value: stats.topContributors.slice(0, 5).map((contributor, index) =>
              `${index + 1}. ${contributor.username} (${contributor.helpfulnessScore.toFixed(1)} score)`
            ).join('\n') || 'No contributors yet',
            inline: true,
          },
          {
            name: '📱 Most Active Channels',
            value: stats.channelActivity.slice(0, 5).map((channel, index) =>
              `${index + 1}. <#${channel.channelId}> (${channel._count.id} messages)`
            ).join('\n') || 'No activity yet',
            inline: false,
          }
        )
      ...
      if (stats.recentActions.length > 0) {
        embed.addFields({
          ...
          value: stats.recentActions.slice(0, 5).map(action =>
            `• ${action.action.replace('_', ' ')} - <t:${Math.floor(action.timestamp.getTime() / 1000)}:R>`
          ).join('\n'),
          inline: false,
        });
      }
  ...
  private async handleModeration(interaction: ChatInputCommandInteraction, serverId: string): Promise<void> {
    try {

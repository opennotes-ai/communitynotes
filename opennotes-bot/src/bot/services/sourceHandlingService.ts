...
export class SourceHandlingService {
  ...
  private static readonly MAX_SOURCES_DISPLAY = 10;
   ...
   */
  private validateSource(source: string): SourceInfo {
    ...
    try {
      ...
      const isBlocked = SourceHandlingService.BLOCKED_DOMAINS.some(blockedDomain =>
   ...
   */
  formatSourcesForEmbed(sources: string[], compact: boolean = false): { value: string; truncated: boolean } {
    if (sources.length === 0) {
  ...
  createSourcePreviewEmbed(sources: string[], noteId: string): EmbedBuilder {
    const embed = new EmbedBuilder()
      ...
      .setDescription('Review the sources provided for this community note')
    ...
    if (invalidSources.length > 0) {
      embed.addFields({
        ...
        value: invalidSources.map((source, index) =>
          `${index + 1}. ${source.url.length > 50 ? source.url.substring(0, 50) + '...' : source.url}`
        ).join('\n'),
        inline: false,
      });
    }
   ...
   */
  private getTrustIndicator(domain: string): string {
    const isTrusted = SourceHandlingService.TRUSTED_DOMAINS.some(trustedDomain =>
   ...
   */
  private analyzeSourceTrust(sources: SourceInfo[]): {
    trustedCount: number;
    ...
    for (const source of sources) {
      const isTrusted = SourceHandlingService.TRUSTED_DOMAINS.some(trustedDomain =>
        source.domain.includes(trustedDomain)
      );
   ...
   */
  private formatValidSources(sources: SourceInfo[]): string {
  ...
  getSourceQualityMetrics(sources: string[]): {
    ...
    else if (qualityScore >= 20) qualityLabel = 'Poor';

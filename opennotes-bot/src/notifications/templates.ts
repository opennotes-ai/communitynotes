...
export class NotePublishedTemplate implements NotificationTemplate {
  ...
  getDescription(data: Record<string, any>): string {
    return `A Community Note has been published for a message you requested context for. The note provides: ${data.noteClassification}`;
  }
...
export class NoteRatingsReceivedTemplate implements NotificationTemplate {
  ...
  getDescription(data: Record<string, any>): string {
    ...
    return `Your Community Note has received ${total} new rating${total > 1 ? 's' : ''}: ${helpfulCount} helpful, ${notHelpfulCount} not helpful (${percentage}% helpful overall)`;
  }
...
export class NoteStatusChangedTemplate implements NotificationTemplate {
  ...
  getDescription(data: Record<string, any>): string {
    ...
    return `Your Community Note status changed from "${oldStatus}" to "${newStatus}". ${this.getStatusDescription(newStatus)}`;
  }
...
export class ContributorMilestoneTemplate implements NotificationTemplate {
  ...
  getDescription(data: Record<string, any>): string {
    ...
    return `Congratulations! You've reached a new milestone: ${milestoneName}. Your ${metric} is now ${value}. Thank you for contributing to Community Notes!`;
  }

...
export class NotificationIntegration {
  ...
  async handleNewRequestThresholdMet(messageId: string): Promise<void> {
    try {
      ...
      for (const contributor of contributors) {
        ...
        await this.notificationService.queueNotification(
          contributor.id,
          NotificationType.NEW_REQUESTS_THRESHOLD_MET,
          notificationData,
  ...
  async handleNotePublishedOnRequest(noteId: string): Promise<void> {
    try {
      // Get note details
      ...
      for (const request of note.message.noteRequests) {
        ...
        await this.notificationService.queueNotification(
          request.requestorId,
          NotificationType.NOTE_PUBLISHED_ON_REQUEST,
          notificationData,
  ...
  async handleNoteReceivedRatings(noteId: string, newRating: { helpful: boolean; raterId: string }): Promise<void> {
    try {
      ...
      await this.notificationService.queueNotification(
        note.authorId,
        NotificationType.NOTE_RECEIVED_RATINGS,
        notificationData,
  ...
  async handleNoteStatusChanged(noteId: string, oldStatus: string, newStatus: string): Promise<void> {
    try {
      const note = await prisma.communityNote.findUnique({
        where: { id: noteId },
        include: {
  ...
  async handleContributorMilestone(userId: string, milestone: {
  ...
  async checkAndTriggerMilestones(userId: string): Promise<void> {
    try {
      ...
      const milestones = [
        { value: 1, name: 'First Community Note', metric: 'community notes written' },
        { value: 5, name: 'Note Contributor', metric: 'community notes written' },
        { value: 10, name: 'Active Contributor', metric: 'community notes written' },
        { value: 25, name: 'Trusted Contributor', metric: 'community notes written' },
        { value: 50, name: 'Expert Contributor', metric: 'community notes written' },
        { value: 100, name: 'Master Contributor', metric: 'community notes written' },
      ];
  ...
  private async getEligibleContributors(serverId: string): Promise<{ id: string }[]> {
    try {
      // Get trusted contributors and active users in this server
  ...
  private shouldNotifyForRatingCount(totalRatings: number): boolean {

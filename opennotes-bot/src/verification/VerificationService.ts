...
export class VerificationService {
  private providers: Map<VerificationMethod, VerificationProvider> = new Map();
  ...
  private users: Map<string, VerifiedUser> = new Map();
  ...
  async startVerification(request: StartVerificationRequest): Promise<VerificationResponse> {
    ...
    try {
      ...
      await this.logSecurityEvent(discordUserId, 'verification_attempt', {
        method,
        target: (provider as any).maskTarget ? (provider as any).maskTarget(target) : target.substring(0, 3) + '***',
        verificationId,
      });
  ...
  async completeVerification(request: CompleteVerificationRequest): Promise<VerificationResponse> {
    ...
    try {
      ...
      return {
        success: true,
        message: 'Verification completed successfully! You can now participate in Community Notes.',
        status: 'verified',
      };
  ...
  async isUserVerified(discordUserId: string): Promise<boolean> {
    const user = this.users.get(discordUserId);
    return user?.isVerified || false;
  }

  async getUserPermissions(discordUserId: string): Promise<UserPermissions> {
    const user = this.users.get(discordUserId);
  ...
  async getVerificationStatus(discordUserId: string): Promise<VerifiedUser | null> {
    return this.users.get(discordUserId) || null;
  }

  private async markUserAsVerified(discordUserId: string, verification: UserVerification): Promise<void> {
    const existingUser = this.users.get(discordUserId);
  ...
  private findPendingVerification(discordUserId: string, method: VerificationMethod): UserVerification | null {
  ...
  private isExpired(verification: UserVerification): boolean {
  ...
  private getEventSeverity(eventType: SecurityEvent['eventType']): SecurityEvent['severity'] {
    switch (eventType) {

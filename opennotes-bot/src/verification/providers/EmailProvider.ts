...
export class EmailVerificationProvider extends BaseVerificationProvider {
  ...
  async sendVerification(email: string, code: string, data?: any): Promise<boolean> {
    ...
    try {
      const subject = data?.subject || 'Verify your Community Notes account';
      const template = this.getEmailTemplate(code, data);

      const mailOptions = {
        from: {
          name: appConfig.SMTP_FROM_NAME || 'Community Notes',
  ...
  private getEmailTemplate(code: string, data?: any): string {
    ...
    if (template === 'default') {
      return `
        ...
        <body>
          <div class="header">
            <h1>Community Notes Verification</h1>
          </div>
          <div class="content">
            <h2>Verify Your Discord Account</h2>
            <p>Someone (hopefully you!) is trying to verify this email address for use with Community Notes on Discord.</p>
            ...
            <p><strong>Note:</strong> This verification links your email to your Discord account for Community Notes participation.</p>
          </div>
          <div class="footer">
            <p>This is an automated message from Community Notes. Please do not reply to this email.</p>
          </div>
        </body>
        </html>
      `;
...
Someone is trying to verify this email address for use with Community Notes on Discord.

Your verification code is: ${code}

This code will expire in 15 minutes. If you didn't request this verification, you can safely ignore this email.

Note: This verification links your email to your Discord account for Community Notes participation.

This is an automated message from Community Notes. Please do not reply to this email.
    `.trim();
  }
}